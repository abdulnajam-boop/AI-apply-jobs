export function HeroVisual() {
  return (
    <div className="relative mx-auto h-[380px] w-full max-w-xl overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900 p-6 shadow-2xl">
      <div className="absolute left-1/2 top-8 h-40 w-40 -translate-x-1/2 rounded-full bg-gradient-to-br from-violet-400/80 to-cyan-300/80 blur-2xl" />

      <div className="absolute right-8 top-10 hidden h-24 w-24 rotate-12 rounded-xl border border-white/20 bg-white/10 p-2 text-white shadow-lg backdrop-blur md:block">
        <p className="text-[10px] uppercase tracking-wider text-cyan-200">Match Score</p>
        <p className="mt-2 text-2xl font-bold">91%</p>
      </div>

      <div className="absolute left-6 top-16 hidden w-40 -rotate-6 rounded-xl border border-white/20 bg-white/10 p-3 text-white shadow-lg backdrop-blur md:block">
        <p className="text-xs font-semibold">Resume Card</p>
        <p className="mt-2 text-[11px] text-slate-200">Frontend Engineer · 6 yrs</p>
      </div>

      <div className="absolute bottom-8 left-6 right-6 grid gap-3 sm:grid-cols-2">
